...
namespace MySpot.Core.Entities
{
    public class WeeklyParkingSpot
    {
        ...
        internal void AddReservation(Reservation reservation, Date now)
        {
            ...
            var alreadyReserved = _reservations.Any(x => x.Date.Value.Date == reservation.Date.Value.Date);

            if (alreadyReserved)
                throw new PakingSpotAlreadyReservedException(reservation.Date.Value.Date, Name);
